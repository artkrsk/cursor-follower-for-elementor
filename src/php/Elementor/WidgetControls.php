...
class WidgetControls {
	...
	/**
	 * The hint a widget shows: WHAT it is, then the one field for that choice. A
	 * hint is text or an icon, never both, so each field is conditioned on the
	 * chooser. Both ride a per-widget var prefix, kept distinct because custom
	 ...
	 * @param array<string, string> $gate  The widget's own enable switcher.
	 */
	private function add_hint_controls(
		...
		// untouched widget reads no var and keeps the rule's own default.
		$element->add_control(
		...
		//
		// Unquoted on purpose — the value may come back as a url() from the filter
		// below, which could not live inside quotes.
		//
