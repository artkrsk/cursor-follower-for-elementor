import { labelFitScale } from '@ts/utils/labelFitScale'
...
describe('labelFitScale', () => {
  ...
  it('fits the diagonal of the label box, not its width', () => {
    ...
    expect(labelFitScale(30, 40, 50, 0)).toBeCloseTo(1)
    // A square label needs more than its own width.
    expect(labelFitScale(50, 50, 50, 0)).toBeGreaterThan(1)
  })

  it('adds the margin on both sides', () => {
    // diagonal 50 + 2×10 = 70, over base 50.
    expect(labelFitScale(30, 40, 50, 10)).toBeCloseTo(70 / 50)
  })

  it('is a ratio of the base size', () => {
    expect(labelFitScale(30, 40, 100, 0)).toBeCloseTo(0.5)
    expect(labelFitScale(30, 40, 25, 0)).toBeCloseTo(2)
  })

  it('returns 0 when there is no base to be a ratio of', () => {
    expect(labelFitScale(30, 40, 0, 10)).toBe(0)
    expect(labelFitScale(30, 40, -1, 10)).toBe(0)
  })

  it('handles an unmeasured label as margin-only', () => {
    expect(labelFitScale(0, 0, 50, 10)).toBeCloseTo(20 / 50)
  })
})
